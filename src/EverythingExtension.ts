    ...
    quickPick.onDidAccept(() => {
      try {
        ...
        if (selectedItem && selectedItem.label.match(/[\\/]/)) {
          ...
          if (type !== "\\") {
            vscode.window.showQuickPick([OPEN_WITH_VSCODE, OPEN_WITH_EXPLORER, OPEN_WITH_DEFAULT_APPLICATION, COPY_PATH_TO_CLIPBOARD], { placeHolder: "Choose how to open the selected file" }).then(async selection => {
              if (!selection) {
                return;
              }
              switch (selection) {
                ...
                case COPY_PATH_TO_CLIPBOARD:
                  ...
                  vscode.window.showInformationMessage(`Path copied to clipboard: ${path}`);
                  break;
              }
            });
          } else {
            vscode.window.showQuickPick([OPEN_WITH_VSCODE, OPEN_WITH_EXPLORER, OPEN_WITH_CMD, OPEN_WITH_POWERSHELL, COPY_PATH_TO_CLIPBOARD], { placeHolder: "Choose how to open the selected folder" }).then(async selection => {
              ...
              switch (selection) {
                ...
                case OPEN_WITH_EXPLORER:
                  ...
                  break;
                case OPEN_WITH_CMD:
                  require("child_process").exec(`start cmd /K "cd /d ${path}"`);
                  break;
                case OPEN_WITH_POWERSHELL:
                  require("child_process").exec(`start powershell -NoExit -Command "cd '${path}'"`);
                  break;
                case COPY_PATH_TO_CLIPBOARD:
                  await vscode.env.clipboard.writeText(path);
                  vscode.window.showInformationMessage(`Path copied to clipboard: ${path}`);
